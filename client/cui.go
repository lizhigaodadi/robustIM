...
func (self VOT) Show(g *gocui.Gui) error {
	...
	fmt.Fprintf(v, "%v:%v%v\n", color.FgGreen.Text(self.Name), self.Sep, color.Yellow.Text(self.Msg))
	return nil
}
...
func doRecv(g *gocui.Gui) {
	/*TODO:receive message handler logic*/
}

func quit(g *gocui.Gui, v *gocui.View) error {
	/*TODO:close chat client logic*/
	return nil
}
...
func viewInput(g *gocui.Gui, x0, y0, x1, y1 int) error {
	if v, err := g.SetView("main", x0, y0, x, y1); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		v.Editable = true
